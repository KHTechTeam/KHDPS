...
def select_reference_files(filenames, run_map, norm_point):
    """Return dict[(electrode, frequency)] -> filename chosen as the 0%
    baseline reference for that electrode/frequency combo.

    - For single/multi-channel potentiostat files (no device ID), the
      reference is the file whose run number exactly matches norm_point,
    ...
    - For internal FC-board files (device ID present), a frequency can be
      ...
      would silently drop it entirely. Instead, the reference is whichever
      cycle that electrode/frequency combo was first scanned in.
    """
    exact_matches = {}    # (elec, freq) -> filename, non-device files at norm_point
    earliest_device = {}  # (elec, freq) -> (run, filename), device files
    ...
    for fname in filenames:
        ...
        if extract_device_id(fname) is None:
            ...
            if key not in earliest_device or run < earliest_device[key][0]:
                earliest_device[key] = (run, fname)

    reference_files = dict(exact_matches)
    for key, (_, fname) in earliest_device.items():
        reference_files.setdefault(key, fname)
...
def run_full_analysis(group_data, norm_point, output_dir, parent=None, use_smoothing: bool = False, keep_frac: float = 0.11):
    ...
    for i, (group, details) in enumerate(group_data.items()):
        ...
        for folder, files in swv_files.items():
            ...
            ref_files = select_reference_files(files, run_map, norm_point)
            for (elec, freq), fname in ref_files.items():
                filepath = os.path.join(folder, fname)
